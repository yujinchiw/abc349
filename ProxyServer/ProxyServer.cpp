...
#include <cstdlib>  // สำหรับการสุ่มเลข

std::vector<DWORD> serverPorts;  // ประกาศตัวแปรเก็บพอร์ต
...
bool ProxyServer::init()
{
    ...
    xField *field = getDBConnPool().getField(RO_DATABASE_NAME, "proxy");
    if (field)
    {
        {
            char where[128];
...

        {
            ...
            if (QWORD_MAX != ret)
            {
                if (ret)
                {
                    ...
                    // เก็บพอร์ตทั้งหมดไว้ใน serverPorts
                    serverPorts.push_back(m_dwGProxyPort);
                }
            }
        }
    }
...
//#ifdef _ALL_SUPER_GM
        ...
        if (!message.has_accid() || !message.has_account() || !message.has_password()){
	    return true;
	}
