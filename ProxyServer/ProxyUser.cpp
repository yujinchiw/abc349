...
bool ProxyUser::connectGateServer(DWORD zoneid)
{
    ...
    if (RedisManager::getMe().getHashAll(key, oData))
    {
        ...
        for (auto &it : list)
        {
            XDBG << "[选择网关]" << "zoneid:" << zoneid << "port:" << it.second << XEND;

            // สุ่มเลือกพอร์ต
            DWORD randomPort = serverPorts[std::rand() % serverPorts.size()];
            xNetProcessor *pTask = NEW xNetProcessor(thisServer->getServerID(), it.second.c_str(), randomPort);
            if (!pTask) continue;

            if (pTask->connect() == false)
            {
                SAFE_DELETE(pTask);
                continue;
            }

            thisServer->addGateServerTask(pTask);
            m_pGateServerTask = pTask;
            m_dwZoneID = zoneid;

            XLOG << "[连接游戏服务器]" << accid << id << name << "zoneid:" << zoneid << "成功" << XEND;
            return true;
        }
    }
    return false;
