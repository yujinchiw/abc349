...
bool GMCommandRuler::additem(xSceneEntryDynamic* entry, const xLuaData& params)
{
  ...
  if (eType != EPACKTYPE_MIN)
  {
    ...
    if (pPack == nullptr)
    {
      XERR << "[GM-AddItem] accid: " << pUser->accid << " failed to find package for PackType: " << eType << XEND;
      return false;
    }

    if (!pPack->addItem(oData, eMethod))
    {
      XERR << "[GM-AddItem] accid: " << pUser->accid << " failed to add item to package. ItemID: " << id << " PackType: " << eType << XEND;
      return false;
    }
    else
    {
      XERR << "[GM-AddItem] accid: " << pUser->accid << " successfully added item to package. ItemID: " << id << " PackType: " << eType << XEND;
      return true;
    }
  }
  else
  {
    if (!pUser->getPackage().addItem(oData, eMethod))
    {
      XERR << "[GM-AddItem] accid: " << pUser->accid << " failed to add item to general package. ItemID: " << id << XEND;
      return false;
    }
    else
    {
      XERR << "[GM-AddItem] accid: " << pUser->accid << " successfully added item to general package. ItemID: " << id << XEND;
      return true;
    }
  }
}
