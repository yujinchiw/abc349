...
void BaseSkill::handleDamage(xSceneEntryDynamic* enemy, xSceneEntryDynamic* attacker, HitedTarget* hit, QWORD targetsNum, SkillRunner& oRunner) const
{
    if (!enemy || !attacker)
        return;
    ...
    //QWORD arrow = oRunner.getArrowID();  // เปลี่ยนเป็น QWORD
    SLuaSkillParam skillparam;
    skillparam.m_dwSkillID = m_dwSkillID;
    skillparam.m_dwTargetNumAndIndex = targetsNum;  // เปลี่ยนเป็น QWORD
    ...
    {
        const map<DWORD, DWORD>& buffs = oRunner.getBuffLayers();  // คงเป็น DWORD
        for (auto& m : buffs)
    ...
    if (damage >= 1000000)
    {
        XERR << "[Skill-Damage Abnormal], Damage value:" << damage << "Attacker:" << attacker->name << attacker->id << "Defender:" << enemy->name << enemy->id << XEND;
        auto printbuf = [&](SBufferData& r)
        {
            XLOG << r.id;
        };
        XLOG << "Attacker's current buffs:";
        attacker->m_oBuff.foreach(printbuf);
        XLOG << XEND;

        XLOG << "Defender's current buffs:";
        enemy->m_oBuff.foreach(printbuf);
        XLOG << XEND;
    }

    if (!skillparam.m_mapID2ShareDam.empty())
    {
        for (auto& m : skillparam.m_mapID2ShareDam)
        {
            ...
            if (pShareTarget == nullptr)
                continue;
            pShareTarget->set_charid(m.first);  // QWORD
            pShareTarget->set_damage(m.second.first);  // QWORD
            pShareTarget->set_type(m.second.second);  // DWORD ยังใช้ได้ถ้าไม่ต้องเกิน 32 บิต
        }
    }
    ...
    {
        QWORD stiff = enemy->m_oBuff.getAutoBlockStiff();  // เปลี่ยนเป็น QWORD
        if (stiff > 0)
    ...
    {
        QWORD stiff = enemy->m_oBuff.getWeaponBlockStiff();  // เปลี่ยนเป็น QWORD
        if (stiff > 0)
            enemy->m_oMove.addBeHitDelayTime(stiff);
    }
    break;
