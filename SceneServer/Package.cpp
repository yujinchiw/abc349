...
XERR << "[包裹-物品删除Test1] AccID: " << m_pUser->accid << " CharID: " << m_pUser->id << " "
  << m_pUser->getProfession() << " " << m_pUser->name
  << getPackageType() << " add reduce item:" << m->second->getTypeID() << " count:" << count
  << " left count :" << m->second->getCount() << " 来源 :" << source << XEND;
